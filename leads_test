...
Инструкция находится во вкладке “Помощь” и называется просто “Инструкция”, в то время как сам файл pdf называется “Руководство по эксплуатации”.
Исходя из текста документа можно сделать вывод, что это “Руководство пользователя по работе с системой LEADS”.
Предлагаю файл для скачивания в “Личном кабинете” назвать “Руководство пользователя”, чтобы у пользователя не возникало недопонимания, нужный ли документ он скачивает.
Первым пунктом в руководстве описан процесс регистрации в системе LEADS.RU, но без регистрации на сайте скачать руководство невозможно. Описание процесса регистрации в руководстве не имеет смысла, потому что пользователь, который скачал руководство — уже зарегистрирован в системе.
...
“Список используемых выражений”: документ, где будут описаны принятые в компании Лидс описания действий пользователя. Например, “нажать на кнопку”, “выбрать строку”, “ввести данные” — вместо любых других вариантов. Это позволит сделать всю техническую документацию Лидс узнаваемой и единообразной. У читателя не будет возникать недопонимания из-за того, что одни и те же действия описываются разными словами.
“Гайд по оформлению”: документ, где будут описаны принятые в компании Лидс основные элементы форматирования. Например, если в гайде будет принято, что названия кнопок пишутся только жирным шрифтом, всплывающие подсказки — только курсивом, и т.п., это значительно облегчит пользователю работу с документацией.
...
Переписать “Введение”, где отразить следующие пункты:
Описать аудиторию: кто основные пользователи сервиса LEADS.RU, какие у них цели, какие результаты они хотят получить, работая с сервисом LEADS.RU. 
Кратко описать основные элементы сервиса и какую работу они выполняют — исходя из целей пользователей.
Указать, что по всем вопросам, ответы на которые пользователь не нашел в руководстве, он может обратиться к персональному менеджеру.
Разбить всю информацию, отраженную в руководстве, на смысловые группы. Предварительно группы могут быть следующими:
*Описание интерфейса: создание площадки, восстановление пароля, персональный менеджер, приглашение реферала, управление подписками, главная страница, статистика.
Работа с продуктом: виды траффика для привлечения лидов, офферы (категории, подключение нового оффера, блок оффера на главной странице), статусы конверсий, статусы веб-мастеров, выплаты.
Сервисы: перенаправления, служба доставки конверсий, агентские кабинеты, парковка домена, отправка анкет (через API, через Excel).*
Объединить группы там, где это возможно. Например, объединить группы “Отправка анкет через API” и “Отправка анкет через Excel” в отдельную группу “Отправка анкеты”. В этой группе создать две подгруппы: “Через API”, “Через Excel”.
Пронумеровать все группы с учетом вложенных подгрупп. Составить содержание с учетом нумерации: это сделает информацию наглядной и облегчит пользователю поиск ответов на его вопросы.
...
Пройдите по ссылке. Вы окажетесь в окне **Вход в личный кабинет**.
Нажмите кнопку *Создайте аккаунт* в нижней части окна. Вы окажетесь в окне **Регистрация**.
Заполните поля регистрационной формы. Нажмите кнопку **Зарегистрироваться** в нижней части окна. Вы окажетесь в личном кабинете. Также на указанную Вами почту придет письмо с данными для входа на сайт.
... *
Привести к единообразию все пункты. Каждый пункт должен отвечать на вопрос “Что сделайте?” (“Пройдите”, “Нажмите”, “Заполните”) или на вопрос “Что сделать?” (“Пройти”, “Нажать”, “Заполнить”). Принять решение о том, на какой вопрос будет отвечать каждый пункт, переписать документ, чтобы в нем не встречались оба варианта.
Расставить перекрестные ссылки внутри документа. Для облегчения работы с руководством, можно проставить ссылки из “Содержания”, чтобы пользователю не пришлось листать до нужной страницы.
Проверить единообразие форматирования: заголовки и подзаголовки должны быть одинакового размера на протяжении всего документа, отступы и пробелы во всем руководстве должны быть одинаковыми.
...
Разработать общую схему для описания всех инструментов. Предварительный пример описания:
*Название инструмента (например, служба доставки конверсий);
Описание инструмента (что он делает);
Какие задачи пользователя можно решить с помощью этого инструмента (сценарии использования);
Виды этого инструмента (типы адреса доставки конверсий);
Пошаговая инструкция для работы с инструментом.*
Переписать описания всех инструментов согласно разработанной схеме.
Пронумеровать и/или подписать все инструкции. При необходимости сделать ссылки на иллюстрации в тексте документации.
Привести к единообразию формы и цвета стрелки и фигурные выделения на иллюстрациях. Следовательно, если в начале документа важные элементы на иллюстрациях были выделены красным овалом, они должны выделяться так на протяжении всего документа.
 
### Редактура документа
После завершения исправлений предлагаю провести следующие мероприятия по редактированию текста и ликвидации ошибок.
От технического писателя:
...
От редактора, другого технического писателя или менеджера:
